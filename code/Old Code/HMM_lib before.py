...
#        print A
        ...
        if (r == 0):
            ...
            for n in range(N):    # For every HMM sequence
                ll = ll + HMMlf.sum_logs(alpha[:,n,T-1]);
#            logl.append(ll);  # Maybe not append coz difference is too much
...
#        print gamma [0,0,:]
        fi = HMMlf.get_fi_matrix_log( I,N,T,A,B, alpha,beta,data );
...
#        print fi [0,0,:]
        ...
        N_gamma = np.sum(np.sum(gamma[:,:,0]));
        N_i_gamma = np.zeros((I,1));
        for i in range(I):
            N_i_gamma[i] = np.sum(gamma[i,:,0]);
...
#        print pi
        ...
        for i in range(I):
            ...
            E_i_fi = np.sum(np.sum(np.sum(fi[i,:,:,:])));
            for j in range(I):
                E_ij_fi = np.sum(np.sum(fi[i,j,:,:]));
...
#        print A
        ...
        for i in range(I): # For every cluster
            ...
            N_i_gamma = np.sum(np.sum(gamma[i,:,:]));
