...
def load_real_data(path = 'data/',file_name='face_scrambling_ERP.mat'):
    ...
    return data,conditions,mat['time']
...
def remove_baseline_data(dataIn):
    data = dataIn.copy()
    """Remove the baseline from the whole trial"""
    for i,row in enumerate(data):
        for j,x in enumerate(row):
            data[i,j] = remove_baseline_subject(x)
    return data

def remove_baseline_subject(Xdata):
    """Remove baseline from subject and transpose"""
    Xbaseline = np.mean(Xdata[0:50],axis=0)
    Xdata = Xdata-Xbaseline #broadcasting the mean baseliene vector
    return Xdata

def cut_beginning_data(dataIn):
    data = dataIn.copy()
    """remove the first 50 points from all trails"""
    for i,row in enumerate(data):
        for j,x in enumerate(row):
            data[i,j] = cut_beginning_subject(x)
    return data

def cut_beginning_subject(Xdata):
...
def normalize_subject(Xdata):
    """Remove the baseline from the whole trial"""
    N,d = Xdata.shape
    for i in range(N):
        Xdata[i,:] = Xdata[i,:]/np.linalg.norm(Xdata[i,:])
    return Xdata
